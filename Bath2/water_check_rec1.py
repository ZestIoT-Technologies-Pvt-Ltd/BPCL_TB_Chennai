...
def water_quality(img1):
	...
	if wat_quality == 0 :
		...
		if wat_check > 4:
			...
			wat_quality =1
			wat_path = water_path+img_dir+"/DESKTOP_BATH2_WATER_QUALITY_BAD_EVENT1_ON_"+img_name
			shutil.move(water_temp+img_name,wat_path)
			print("***************************** Water Quality not good ***************************")
			try:
				sc=ClientSocket(device_id=str('BPCL_BATH2_NX_0001'))
		...
		if wat_rectify >4 :
			wat_quality =0
			img_dir=(datetime.now()).strftime("%Y_%m_%d")
			shutil.move(water_temp+img_name,water_path+img_dir+"/DESKTOP_BATH2_WATER_QUALITY_RESTORED_EVENT1_OFF_"+img_name)
			water_rectify()
			wat_rectify=0
			wat_flag = 10
